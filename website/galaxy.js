...
var Config = {
    ...
    "opensea_collection":"https://testnets.opensea.io/collection/exoplanets-3",
...
function popupSystem(star) {
	...
	<div class="tableName">${game.official_designation} ${game.planetcount} Planets</div>
	<div class="close clickable" onclick='window.popDown()'>x</div>
	<table class="solarSystem">
	<tr>
	<th title="${labels.getLabel('official')}">Official<br>Designation</th>
    ...
    <tr>
        <td><a target="_blank" href="${Config.opensea_template.replace('chain', planet.chain).replace('contract', planet.contract).replace('tokenid', planet.tokenid)}">Purchase planet</a></td>
        <td>0.3 ETH</td>
    </tr>
    </table>`;
...
window.lifeformImage = function(planet) {
	...
	<img src="${lifeformImage}" id="lifeformImage" width="100%" class="lifeFormImage rounded" style="position: relative;">`;
    let annotation = document.createElement("div")
    Object.assign(annotation.style, {position: 'absolute', top: '1%', left: '5%'});
...
function loadPlanets(planets) {
    const system_radius = Config.system_sphere_radius || 10;
	...
	if (! system.userData.game.stars[planet.star_index]) {
		...
		"label": addLabel(starSprite, starLocation, `${planet.star_index} ${planet.star_name}`,
...
// load planets
await fetch("planets.json").then((response) => response.json())
