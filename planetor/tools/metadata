...
if __name__ == "__main__":
    if len(sys.argv) < 1:
        print('metadata.py extract metadata from all mp4 files, metadata.py "string with metadata variables" filespec' )
        print('usage example: metadata.py format="planet: {id} uses this {clouds} map and has {moons} moons" *.mp4')
        print("metadata fields available: all,id,planet_size,camera_location,camera_angle,camera_look_at,sun_brightness,sun_position,background,planet,clouds_size,clouds,clouds_density,atmosphere,atmosphere_composition,atmosphere_density,atmosphere_size,moons,moon_position,moon_size,moon,rings,random_color,random_float,random_trans,star_system,star_index,planet_index,evaluation,evaluation2,identity,lifeform,chemistry,planet_type,unexplored,artist,rare,radioactives,refractories,industrials,specialized,biologicals,exotics,relics,resources_value,created")
        sys.exit(0);
    ...
    format = params.get('format') or "{all}"
