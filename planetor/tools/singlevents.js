...
function help() {
    ...
    where command is:
      ...
      event - send events to the planets matching the selectors
...
async function Process() {
	...
	case "event": {
		...
		if (finds.length > 0) {
		    event = finds[0];
		    Utils.message(3, `Already Existing, modify it ${event.get("identity")}`);
		}
		else
		{
		    Utils.message(3, `Create New Event from file ${filename}`);
